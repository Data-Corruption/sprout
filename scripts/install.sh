...
bin_url_sha256="${RELEASE_URL}${BIN_ASSET_NAME_SHA256}"
...
curl $curl_opts -o "$hash_out" "$bin_url_sha256" || { rc=$?; fatalf 'Download of checksum failed (rc=%d)' "$rc"; }
...
default_port=$(printf '%s' "$build_vars" | sed -n 's/.*"defaultPort":\([0-9]*\).*/\1/p')
